...
const PrinterSetup = () => {
  ...
  //   try {
  //     const data = await axiosInstance.post('/printers/testPrint', {
...
const handleTestPrint = async (printer) => {
  const uniqueKey = printer.id || printer._id || printer.printer_id;
  const ip = (printer?.ip_address || "").trim();
  const pid = printer?.printer_id || printer?.id || printer?._id;
  const port = Number(printer?.port) || 9100;

  if (!uniqueKey) {
    toast.error('Printer record missing unique id — cannot test.');
    return;
  }

  // if no IP: instead of immediately showing an error, attempt a fallback
  if (!ip) {
    // OPTION A: call a "local print" backend endpoint (recommended if you will support USB/local prints)
    try {
      setPrinters(prev =>
        prev.map(p => (p.id === uniqueKey || p._id === uniqueKey || p.printer_id === uniqueKey) ? { ...p, status: 'testing' } : p)
      );

      // backend should implement /printers/testPrintLocal and accept { printer_id }
      const { data } = await axiosInstance.post('/printers/testPrint', { printer_id: pid }, { timeout: 20000 });
       console.log(data,"nand")
      if (data && data.success) {
        toast.success(`Test print (local) sent to ${printer.name || pid}`);
        setPrinters(prev =>
          prev.map(p => (p.id === uniqueKey || p._id === uniqueKey || p.printer_id === uniqueKey) ? { ...p, status: 'online' } : p)
        );
      } else {
        const reason = data && data.message ? data.message : 'Unknown error';
        toast.error(`Local test print failed for ${printer.name || pid}: ${reason}`);
        setPrinters(prev =>
          prev.map(p => (p.id === uniqueKey || p._id === uniqueKey || p.printer_id === uniqueKey) ? { ...p, status: 'error' } : p)
        );
      }
    } catch (err) {
      const msg = err?.response?.data?.message || err?.message || 'Request failed';
      toast.error(`Local test print failed for ${printer.name || pid}: ${msg}`);
      console.error('Local test print error:', err);
      setPrinters(prev =>
        prev.map(p => (p.id === uniqueKey || p._id === uniqueKey || p.printer_id === uniqueKey) ? { ...p, status: 'offline' } : p)
      );
    }
    return;
    /*
    // OPTION B: If you prefer to keep current behavior (force user to add IP),
    // open Edit modal instead of sending a request. Uncomment and use:
    setEditingPrinter({ ...printer });
    setShowEditPrinterModal(true);
    return;
    */
  ...
  // Normal network printing path
  const payload = { printer_id: pid, ip_address: ip, port };
  ...
  try {
    const { data } = await axiosInstance.post('/printers/testPrint', payload, { timeout: 20000 });
      console.log(payload,"naxch")
    if (data && data.success) {
      toast.success(`Test page sent to ${printer.name || pid}`);
      setPrinters(prev =>
        prev.map(p => (p.id === uniqueKey || p._id === uniqueKey || p.printer_id === uniqueKey) ? { ...p, status: 'online' } : p)
      );
    } else {
      const reason = data && data.message ? data.message : 'Unknown error';
      toast.error(`Failed to print on ${printer.name || pid}: ${reason}`);
      setPrinters(prev =>
        prev.map(p => (p.id === uniqueKey || p._id === uniqueKey || p.printer_id === uniqueKey) ? { ...p, status: 'error' } : p)
      );
    }
    ...
    const msg = err?.response?.data?.message || err?.message || 'Request failed';
    toast.error(`Failed to print on ${printer.name || pid}: ${msg}`);
